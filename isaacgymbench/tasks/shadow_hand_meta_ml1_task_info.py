...
def obtrain_task_info(task_name):
    ...
    if task_name == "catch_overarm_glue":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0] #pos:x,y,z;euler:z,y,x
        another_hand_start_pose = [0, -0.6, 0.5, 1.57+0.1, 3.14, 3.1415]
        object_pose = [0, 0.04, 0.971, 0, 0, 0]
        goal_pose = [0, -0.6-0.02, 0.98-0.02, 0, 0, 0]
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
    ...
    if task_name == "catch_overarm_lemon":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0] #pos:x,y,z;euler:z,y,x
        another_hand_start_pose = [0, -0.6, 0.5, 1.57+0.1, 3.14, 3.1415]
        object_pose = [-0.01, 0.03, 0.96, 0, 0, 0]
        goal_pose = [0, -0.6-0.02, 0.98-0.02, 0, 0, 0]
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
    ...
    if task_name == "catch_overarm_orange":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0] #pos:x,y,z;euler:z,y,x
        another_hand_start_pose = [0, -0.6, 0.5, 1.57+0.1, 3.14, 3.1415]
        object_pose = [-0.02, 0.04, 0.975, 0, 0, 0]
        goal_pose = [0, -0.6-0.02, 0.98-0.02, 0, 0, 0]
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
...
# 正常处
    ...
    if task_name == "catch_overarm2abreast_scissors":
        ...
        object_asset_options.density = 500
        object_type = "scissors" 
    if task_name == "catch_overarm2abreast_bottle":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "bottle"
    if task_name == "catch_overarm2abreast_bluecup":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.03, 0.979, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "bluecup"
    if task_name == "catch_overarm2abreast_plate":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "plate"
    if task_name == "catch_overarm2abreast_teabox":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "teabox"
    if task_name == "catch_overarm2abreast_clenser":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "clenser"
    if task_name == "catch_overarm2abreast_conditioner":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "conditioner"
    if task_name == "catch_overarm2abreast_correctionfluid":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "correctionfluid"
    if task_name == "catch_overarm2abreast_crackerbox":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.03, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "crackerbox"
    if task_name == "catch_overarm2abreast_doraemonbowl":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "doraemonbowl"
    if task_name == "catch_overarm2abreast_largeclamp":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "largeclamp"
    if task_name == "catch_overarm2abreast_flatscrewdrive":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "flatscrewdrive"
    if task_name == "catch_overarm2abreast_fork":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.03, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "fork"
    if task_name == "catch_overarm2abreast_glue":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0.002, 0.029, 0.981, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "glue"
    if task_name == "catch_overarm2abreast_liption":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.048, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "liption"
    if task_name == "catch_overarm2abreast_lemon":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.021, 0.97, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "lemon"
    if task_name == "catch_overarm2abreast_orange":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.045, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "orange"
    if task_name == "catch_overarm2abreast_remotecontroller":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.03, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "remotecontroller"
    if task_name == "catch_overarm2abreast_sugerbox":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "sugerbox"
    if task_name == "catch_overarm2abreast_repellent":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.045, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "repellent"
    if task_name == "catch_overarm2abreast_shampoo":
        hand_start_pose = [0, 0, 0.5, 1.57 +0.1, 3.14, 0]
        another_hand_start_pose = _overarm2abreast_another_hand_start_pose
        object_pose = [0, 0.05, 0.98, 0, 0, 0]
        goal_pose = _overarm2abreast_goal_pose
    ...
    if task_name == "catch_under2overarm_scissors":
        ...
        object_asset_options.density = 500
        object_type = "scissors"
    if task_name == "catch_under2overarm_bottle":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "bottle"
    if task_name == "catch_under2overarm_bluecup":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "bluecup"
    if task_name == "catch_under2overarm_plate":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "plate"
    if task_name == "catch_under2overarm_teabox":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "teabox"
    if task_name == "catch_under2overarm_clenser":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "clenser"
    if task_name == "catch_under2overarm_conditioner":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "conditioner"
    if task_name == "catch_under2overarm_correctionfluid":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "correctionfluid"
    if task_name == "catch_under2overarm_crackerbox":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "crackerbox"
    if task_name == "catch_under2overarm_doraemonbowl":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "doraemonbowl"
    if task_name == "catch_under2overarm_largeclamp":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "largeclamp"
    if task_name == "catch_under2overarm_flatscrewdrive":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "flatscrewdrive"
    if task_name == "catch_under2overarm_fork":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "fork"
    if task_name == "catch_under2overarm_glue":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "glue"
    if task_name == "catch_under2overarm_liption":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "liption"
    if task_name == "catch_under2overarm_lemon":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "lemon"
    if task_name == "catch_under2overarm_orange":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "orange"
    if task_name == "catch_under2overarm_remotecontroller":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "remotecontroller"
    if task_name == "catch_under2overarm_sugerbox":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "sugerbox"
    if task_name == "catch_under2overarm_repellent":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "repellent"
    if task_name == "catch_under2overarm_shampoo":
        hand_start_pose = [0, 0, 0.5, 0, 0, 0]
        another_hand_start_pose = _under2overarm_another_hand_start_pose
        object_pose = [0-0.01, -0.48, 0.54+0.02, 0, 0, 0]
        goal_pose = _under2overarm_goal_pose
    ...
    _abreastout45_hand_start_pose = [0, -0.55, 0.5, 0, 0.3925, -1.57+0.78]
    _abreastout45_another_hand_start_pose = [0, -1.42, 0.50, 0, -0.3925, -1.57-0.78]   # zyx
    _abreastout45_object_pose = [-0.39+0.07, -0.55-0.30, 0.54, 0, 0, 0]
    _abreastout45_goal_pose = [-0.41+0.11, -1.52+0.26, 0.45, 0, 0, 0]
    
    # 正常处
    if task_name == "catch_abreastout45_poker":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "poker"
    if task_name == "catch_abreastout45_banana":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "banana"
    if task_name == "catch_abreastout45_suger":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "suger"
    if task_name == "catch_abreastout45_apple":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "apple"
    if task_name == "catch_abreastout45_pie":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pie"
    if task_name == "catch_abreastout45_mug":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "mug"
    if task_name == "catch_abreastout45_stapler":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "stapler"
    if task_name == "catch_abreastout45_bowl":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "bowl"
    if task_name == "catch_abreastout45_peach":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "peach"
    if task_name == "catch_abreastout45_pear":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pear"
    if task_name == "catch_abreastout45_strawberry":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "strawberry"
    if task_name == "catch_abreastout45_pen":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pen"
    if task_name == "catch_abreastout45_washer":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "washer"
    if task_name == "catch_abreastout45_scissors":
        hand_start_pose = _abreastout45_hand_start_pose
        another_hand_start_pose = _abreastout45_another_hand_start_pose
        object_pose = _abreastout45_object_pose
        goal_pose = _abreastout45_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymap2i.AssetOptions()
        object_asset_options.density = 500
        object_type = "scissors"
    
    _overarmlr45_hand_start_pose = [-0.15, 0, 0.5, 1.57+0.1, 3.14+0.775, 0.+0.775]
    _overarmlr45_another_hand_start_pose = [+0.15, -0.1, 0.5, 1.57+0.1, 3.14+0.775, 3.1415+0.775]   # zyx
    _overarmlr45_object_pose = [0+0.07, 0.04+0.20, 0.97+0.02-0.10, 0, 0, 0]
    _overarmlr45_goal_pose = [0+0.05, -0.1-0.02-0.24, 0.98+0.01-0.25, 0, 0, 0]     # z= **-0.2/-0.5
    
    # 正常处
    if task_name == "catch_overarmlr45_poker":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        table_pose_dim = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_type = "poker"
    if task_name == "catch_overarmlr45_banana":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "banana"
    if task_name == "catch_overarmlr45_suger":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "suger"
    if task_name == "catch_overarmlr45_apple":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "apple"
    if task_name == "catch_overarmlr45_pie":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pie"
    if task_name == "catch_overarmlr45_mug":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "mug"
    if task_name == "catch_overarmlr45_stapler":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "stapler"
    if task_name == "catch_overarmlr45_bowl":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "bowl"
    if task_name == "catch_overarmlr45_peach":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "peach"
    if task_name == "catch_overarmlr45_pear":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pear"
    if task_name == "catch_overarmlr45_strawberry":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "strawberry"
    if task_name == "catch_overarmlr45_pen":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "pen"
    if task_name == "catch_overarmlr45_washer":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
        ...
        object_asset_options.density = 500
        object_type = "washer"
    if task_name == "catch_overarmlr45_scissors":
        hand_start_pose = _overarmlr45_hand_start_pose
        another_hand_start_pose = _overarmlr45_another_hand_start_pose
        object_pose = _overarmlr45_object_pose
        goal_pose = _overarmlr45_goal_pose
...
def compute_hand_reward_catch_overarmout45(
    ...
    # Find out which envs hit the goal and update successes count
    goal_resets = torch.where(torch.abs(goal_dist) <= 0.0, torch.ones_like(reset_goal_buf), reset_goal_buf)
    successes = successes + goal_resets
    ...
    return reward, resets, goal_resets, progress_buf, successes, cons_successes
@torch.jit.script
def compute_hand_reward_catch_abreastout45(
    rew_buf, reset_buf, reset_goal_buf, progress_buf, successes, consecutive_successes,
    max_episode_length: float, object_pos, object_rot, target_pos, target_rot, left_hand_base_pos, right_hand_base_pos,
    dist_reward_scale: float, rot_reward_scale: float, rot_eps: float,
    actions, action_penalty_scale: float,
    ...
    # resets = torch.where(right_hand_pos[:, 1] <= -0.7, torch.ones_like(reset_buf), reset_buf)
    # resets = torch.where(right_hand_pos[:, 2] >= 0.7, torch.ones_like(resets), resets)
    right_hand_base_dist = torch.norm(right_hand_base_pos - torch.tensor([-0.03, -0.55, 0.5], dtype=torch.float, device=device), p=2, dim=-1)
    left_hand_base_dist = torch.norm(left_hand_base_pos - torch.tensor([-0.03, -1.52, 0.5], dtype=torch.float, device=device), p=2, dim=-1)

    resets = torch.where(right_hand_base_dist >= 0.3, torch.ones_like(reset_buf), reset_buf)
    resets = torch.where(left_hand_base_dist >= 0.3, torch.ones_like(resets), resets)
    ...
    return reward, resets, goal_resets, progress_buf, successes, cons_successes
@torch.jit.script
def compute_hand_reward_catch_overarmlr45(
    rew_buf, reset_buf, reset_goal_buf, progress_buf, successes, consecutive_successes,
    max_episode_length: float, object_pos, object_rot, target_pos, target_rot, left_hand_base_pos, right_hand_base_pos,
    dist_reward_scale: float, rot_reward_scale: float, rot_eps: float,
    actions, action_penalty_scale: float,
    ...
    # Check env termination conditions, including maximum success number
    right_hand_base_dist = torch.norm(right_hand_base_pos - torch.tensor([-0.15, 0.0, 0.5], dtype=torch.float, device=device), p=2, dim=-1)
    
    # small objects
    left_hand_base_dist = torch.norm(left_hand_base_pos - torch.tensor([0.15, -0.1, 0.5], dtype=torch.float, device=device), p=2, dim=-1)  # -0.8
    # big objects
    # left_hand_base_dist = torch.norm(left_hand_base_pos - torch.tensor([0.0, -0.6, 0.5], dtype=torch.float, device="cuda:0"), p=2, dim=-1)  # -0.8
    # Fall penalty x, y and z: distance to the goal is larger than a threashold
    goal_dist_x = torch.abs(target_pos[:, 0] - object_pos[:, 0])
    ...
    resets = torch.where(object_pos[:, 2] >= 1.4, torch.ones_like(resets), resets)
    # X
    resets = torch.where(goal_dist_x >= 0.4, torch.ones_like(resets), resets)
    # Y
    resets = torch.where(object_pos[:, 1] <= -0.6, torch.ones_like(resets), resets)
    resets = torch.where(object_pos[:, 1] >= 0.5, torch.ones_like(resets), resets)
    # # Fall penalty New:
    # resets_near = torch.where(goal_dist_y <= 0.15, torch.ones_like(resets), resets)
    # resets_near_fall = torch.where(object_pos[:, 2] <= 0.8, torch.ones_like(resets), resets)
    # resets = resets_near * resets_near_fall
