...
from algorithms.sarl.ppo_max.normalization import *
...
# some trick:[policy entropy:set entropy_coef=0.01, orignal is 0.0; ]
    def __init__(self,
        ...
        # Normalization
        self.obs_norm = Normalization(num_envs=self.vec_env.num_envs, shape=self.vec_env.num_obs, device=self.device)
        self.reward_scaling = RewardScaling(num_envs=self.vec_env.num_envs, shape=1, gamma=self.gamma, device=self.device)
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        current_obs = self.obs_norm(current_obs)
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    next_obs = self.obs_norm(next_obs)
                    rews = self.reward_scaling(rews.unsqueeze(1)).squeeze(-1)
