...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, states_shape, actions_shape, device='cpu', sampler='sequential'):
        ...
        self.intrisic_stable_standard = torch.zeros(num_envs, 1, device=self.device)
        # Core
    ...
    def compute_returns(self, last_values, gamma, lam):
        advantage = 0
        intrisic_derivatives = 0
        intrisic_advantage = 0
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            ## add lyapunov stability
            intrisic_derivatives = 1 * (next_values - self.values[step])
            intrisic_advantage = self.belta * torch.min(self.intrisic_stable_standard, intrisic_derivatives) + (1 - self.belta) * advantage
            # self.returns[step] = advantage + self.values[step]
            self.returns[step] = intrisic_advantage + self.values[step]
