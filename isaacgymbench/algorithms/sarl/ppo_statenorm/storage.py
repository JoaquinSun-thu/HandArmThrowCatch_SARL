...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, states_shape, actions_shape, device='cpu', sampler='sequential'):
        ...
        self.belta = 0.2 # belta is in [0,1]
        self.stable_standard = torch.zeros(num_envs, 1, device=self.device)
        # Core
    ...
    def compute_returns(self, last_values, gamma, lam):
        advantage = 0
        lyap_derivatives = 0
        lyap_derivatives_step1 = 0
        lyap_derivatives_step2 = 0
        lyap_derivatives_step3 = 0
        lyap_advantage = 0
        
        for step in reversed(range(self.num_transitions_per_env)):
            if step == self.num_transitions_per_env - 3:
                next_values3 = last_values
            elif step == self.num_transitions_per_env - 2:
                next_values3 = last_values
                next_values2 = last_values
            elif step == self.num_transitions_per_env - 1:
                next_values3 = last_values
                next_values2 = last_values
                next_values = last_values
            else:
                next_values3 = self.values[step + 3]
                next_values2 = self.values[step + 2]
                next_values = self.values[step + 1]
            next_is_not_terminal = 1.0 - self.dones[step].float()
            delta = self.rewards[step] + next_is_not_terminal * gamma * next_values - self.values[step]
            advantage = delta + next_is_not_terminal * gamma * lam * advantage
            ## add lyapunov stability
            lyap_derivatives_step1 = 1 * (next_values - self.values[step])
            lyap_derivatives_step2 = (1./2) * (next_values2 - self.values[step])
            lyap_derivatives_step3 = (1./3) * (next_values3 - self.values[step])
            lyap_derivatives = (lyap_derivatives_step1 + lyap_derivatives_step2 + lyap_derivatives_step3)/3.
            lyap_advantage = self.belta * torch.min(self.stable_standard, -lyap_derivatives) + (1 - self.belta) * advantage
            # self.returns[step] = advantage + self.values[step]
            self.returns[step] = lyap_advantage + self.values[step]
