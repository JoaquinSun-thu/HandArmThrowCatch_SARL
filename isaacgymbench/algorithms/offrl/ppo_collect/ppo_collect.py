...
class PPO:
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing:
            ...
            # print(sum(reward_sum)/(len(reward_sum)//(self.vec_env.max_episode_length * self.vec_env.num_envs)*self.vec_env.num_envs)
            print(sum(reward_sum)/(np.array(np.nonzero(np.array(reward_sum))).shape[1]))
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    if it > int(num_learning_iterations * 2/3):
                ...
                if it == int(num_learning_iterations -1 ):
                    states_off = np.vstack(states_off)
                    next_states_off = np.vstack(next_states_off)
                    actions_off = np.vstack(actions_off)
                    rewards_off = np.vstack(rewards_off)
