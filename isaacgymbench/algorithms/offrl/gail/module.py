...
class Disc_Net(nn.Module):
	def __init__(self, obs_dim ,state_dim, action_dim,model_cfg):
		super(Disc_Net, self).__init__()
        
		if model_cfg is None:
			disc_hidden_dim = [256, 256, 256]
			activation = get_activation("tanh")
		else:
			disc_hidden_dim = model_cfg['disc_hid_sizes']
			activation = get_activation(model_cfg['disc_activation'])

		disc_layers = []

		disc_layers.append(nn.Linear(obs_dim+action_dim, disc_hidden_dim[0]))
		disc_layers.append(activation)
		for l in range(len(disc_hidden_dim)):
			if l == len(disc_hidden_dim) - 1:
				disc_layers.append(nn.Linear(disc_hidden_dim[l], 1))
				disc_layers.append(activation)
			else:
				disc_layers.append(nn.Linear(disc_hidden_dim[l], disc_hidden_dim[l + 1]))
				disc_layers.append(activation)

		self.disc = nn.Sequential(*disc_layers)
		disc_weights = [np.sqrt(2)] * len(disc_hidden_dim)
		disc_weights.append(1.0)
		self.init_weights(self.disc, disc_weights)
