...
from algorithms.offrl.gail import RolloutStorage
from algorithms.offrl.gail import ExpertBuffer
...
class GAIL:
    ...
    def __init__(self,
        ...
        self.disc = disc_class(self.observation_space.shape[0], self.state_space.shape[0], self.action_space.shape[0],model_cfg)
        self.disc.to(self.device)

        self.disc_optim = optim.Adam(self.disc.parameters(), lr=self.disc_lr) 
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing:
            ...
            self.expert_buffer = ExpertBuffer(self.state_space.shape[0], self.action_space.shape[0], self.device)
            self.expert_buffer.convert(self.data_dir)    
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    # re-process the reward
                    disc_rews = self.disc_reward(current_obs,actions)

                    # Record the transition (reward = disc_rews)
                    self.storage.add_transitions(current_obs, current_states, actions, disc_rews, dones, values, actions_log_prob, mu, sigma)
                    current_obs.copy_(next_obs)
                ...
                start = stop
                # learning disc net
                mean_disc_loss, mean_acc_pi, mean_acc_exp = self.disc_update()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Loss/Disc_net', locs['mean_disc_loss'], locs['it'])
    ...
    def disc_update(self):
        ...
        acc_pis = []
        acc_exps = []
        for it in range(self.disc_update_num):
            e_state, e_action, _, _, _ = self.expert_buffer.sample(self.disc_batch_size)
            state, action = self.storage.sample(self.disc_batch_size)

            logits_pi = self.disc(state,action)
            logits_exp = self.disc(e_state,e_action)
        ...
        mean_disc_loss = np.mean (losses_disc)
        mean_acc_pi = np.mean(acc_pis)
        mean_acc_exp = np.mean(acc_exps)

        return mean_disc_loss, mean_acc_pi, mean_acc_exp

    def disc_reward(self,current_states,actions):
        
        output = -self.disc(current_states,actions)
        rewards = -F.logsigmoid(output)
