...
def plot_ax(
    ...
    def legend_fn(x):
        ...
        return re.search(legend_pattern, x).group() # return re.search(legend_pattern, x).group(0)
    ...
    # to plot
    # for compare
    # new_file_lists = [file_lists[2],file_lists[1],file_lists[0],file_lists[3],file_lists[4]]
    # new_file_lists = [file_lists[2],file_lists[3],file_lists[1],file_lists[0],file_lists[4],file_lists[5]]
    # for ablation
    # new_file_lists = [file_lists[1],file_lists[2],file_lists[4],file_lists[3],file_lists[0]]
    # for ablation_new
    # new_file_lists = [file_lists[4],file_lists[1],file_lists[5],file_lists[2],file_lists[3],file_lists[0]]
    # for ablation_new2
    # new_file_lists = [file_lists[6],file_lists[5],file_lists[1],file_lists[7],file_lists[2],file_lists[4],file_lists[3],file_lists[0]]
    # for stable_compare
    new_file_lists = [file_lists[1],file_lists[0]]
    ...
    for index, csv_file in enumerate(file_lists):
        ...
        ax.plot(x[:], y[:], color=color, label=LEGEND[index])
        if shaded_std and ykey + ':shaded' in csv_dict:
            y_shaded = smooth(csv_dict[ykey + ':shaded'], radius=smooth_radius)
            ax.fill_between(x[:], y[:] - y_shaded[:], y[:] + y_shaded[:], color=color, alpha=.2) #,  label=True
    # for 10000        
    # for index, csv_file in enumerate(file_lists):
    #     csv_dict = csv2numpy(csv_file)
    #     x, y = csv_dict[xkey], csv_dict[ykey]
    #     y = smooth(y, radius=smooth_radius)
    #     color = COLORS[index % len(COLORS)]
    #     ax.plot(x[:10000], y[:10000], color=color, label=LEGEND[index])
    #     if shaded_std and ykey + ':shaded' in csv_dict:
    #         y_shaded = smooth(csv_dict[ykey + ':shaded'], radius=smooth_radius)
    #         ax.fill_between(x[:10000], y[:10000] - y_shaded[:10000], y[:10000] + y_shaded[:10000], color=color, alpha=.2) #,  label=True
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--xlabel', default='Training Step', help='matplotlib figure xlabel'
    )# Environment Step
    parser.add_argument(
    ...
    parser.add_argument('--root-dir', default='/home/shengjie/RL/isaacgym_bench/isaacgymbench/logs/shadow_hand_catch_abreast', help='root dir (default: ./)') # /home/syqi/RL/DexterousHandEnvs/dexteroushandenvs/logs/shadow_hand_over
    parser.add_argument(
        '--file-pattern',
        type=str,
        default=r".*/test_rew_\d+seeds.csv$",  #rew // std
    ...
    parser.add_argument(
        '--output-path', type=str, help='figure save path', default="/home/shengjie/RL/isaacgym_bench/isaacgymbench/logs/shadow_hand_catch_abreast/figure.png" #/home/syqi/RL/DexterousHandEnvs/dexteroushandenvs/logs/shadow_hand_over/figure.png
    )
    parser.add_argument(
        '--dpi', type=int, default=300, help='figure dpi (default: 200)'
    )
