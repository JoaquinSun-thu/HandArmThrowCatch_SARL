...
def process_gail(args, env, cfg_train, logdir):
    from algorithms.offrl.gail import GAIL, ActorCritic,Disc_Net
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
    ...
    gail = GAIL(vec_env=env,
              ...
              disc_lr = learn_cfg['disc_lr'],
              disc_update_num = learn_cfg['disc_update_num'],
              disc_batch_size = learn_cfg['disc_batch_size'],
              datatype = args.datatype
              )

    if is_testing and args.model_dir != "":
        print("Loading model from {}".format(chkpt_path))
        gail.test(chkpt_path)
    elif args.model_dir != "":
        print("Loading model from {}".format(chkpt_path))
        gail.load(chkpt_path)
