...
def process_ppo_pnorm(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_pnorm import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_max(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_max import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_stability(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_stability import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_add_intrisic(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_add_intrisic import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_add_stability(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_add_stability import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
    ...
    return ppo
def process_ppo_add_stability_constrain(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_add_stability_constrain import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_add_feature(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_add_feature import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_statenorm(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_statenorm import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
    ...
    return ppo
def process_ppo_lyp(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_lyp import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_lyp_constrain(args, env, cfg_train, logdir):
    from algorithms.sarl.ppo_lyp_constrain import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
